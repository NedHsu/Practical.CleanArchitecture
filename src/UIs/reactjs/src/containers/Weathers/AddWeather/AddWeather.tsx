...
class AddWeather extends Component<Props, any> {
  state = {
    title: "Add Weather",
    controls: {
      name: {
  ...
  componentDidMount() {
    ...
    if (id) {
      this.setState({ title: "Edit Weather" });
  ...
  onSubmit = event => {
    ...
    for (let fieldName in this.state.controls) {
      isValid =
        this.checkFieldValidity(fieldName, this.props.weather[fieldName]) &&
        isValid;
    }

    if (isValid) {
      this.props.saveWeather(this.props.weather);
    }
  };
            ...
            <div className="form-group row">
              ...
              <div className="col-sm-10">
                <input
                  ...
                  value={this.props.weather?.description}
                  onChange={event => this.fieldChanged(event)}
                />
                <span className="invalid-feedback">
