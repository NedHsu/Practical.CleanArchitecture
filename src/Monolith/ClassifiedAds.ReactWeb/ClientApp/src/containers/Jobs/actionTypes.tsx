export const FETCH_JOBS = "FETCH_JOBS";
export const FETCH_JOBS_START = "FETCH_JOBS_START";
export const FETCH_JOBS_SUCCESS = "FETCH_JOBS_SUCCESS";
export const FETCH_JOBS_FAIL = "FETCH_JOBS_FAIL";

export const FETCH_JOB = "FETCH_JOB";
export const FETCH_JOB_START = "FETCH_JOB_START";
export const FETCH_JOB_SUCCESS = "FETCH_JOB_SUCCESS";
export const FETCH_JOB_FAIL = "FETCH_JOB_FAIL";

export const UPDATE_JOB = "UPDATE_JOB";
export const RESET_JOB = "RESET_JOB";

export const SAVE_JOB = "SAVE_JOB";
export const SAVE_JOB_START = "SAVE_JOB_START";
export const SAVE_JOB_SUCCESS = "SAVE_JOB_SUCCESS";
export const SAVE_JOB_FAIL = "SAVE_JOB_FAIL";

export const DELETE_JOB = "DELETE_JOB";
export const DELETE_JOB_START = "DELETE_JOB_START";
export const DELETE_JOB_SUCCESS = "DELETE_JOB_SUCCESS";
export const DELETE_JOB_FAIL = "DELETE_JOB_FAIL";

//--export
export const FETCH_JOB_SRCS_EX = "FETCH_JOB_SRCS_EX";
export const FETCH_JOB_SRCS_EX_START = "FETCH_JOB_SRCS_EX_START";
export const FETCH_JOB_SRCS_EX_SUCCESS = "FETCH_JOB_SRCS_EX_SUCCESS";
export const FETCH_JOB_SRCS_EX_FAIL = "FETCH_JOB_SRCS_EX_FAIL";