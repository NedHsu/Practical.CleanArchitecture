...
class AddStockNew extends Component<Props, any> {
  state = {
    title: "Add StockNew",
    controls: {
      name: {
  ...
  componentDidMount() {
    this.props.resetStockNew();
    const id = this.props.match?.params?.id;
    if (id) {
      this.setState({ title: "Edit StockNew" });
      this.props.fetchStockNew(id);
    }
  }
  ...
  fieldChanged = event => {
    ...
    this.props.updateStockNew(stockNew);
  };
  ...
  onSubmit = event => {
    ...
    for (let fieldName in this.state.controls) {
      isValid =
        this.checkFieldValidity(fieldName, this.props.stockNew[fieldName]) &&
        isValid;
    }

    if (isValid) {
      this.props.saveStockNew(this.props.stockNew);
    }
  };
  ...
  render() {
            ...
            <div className="form-group row">
              ...
              <div className="col-sm-10">
                <input
                  ...
                  value={this.props.stockNew?.name}
            ...
            <div className="form-group row">
              ...
              <div className="col-sm-10">
                <input
                  ...
                  value={this.props.stockNew?.code}
                  onChange={event => this.fieldChanged(event)}
                />
                <span className="invalid-feedback">
            ...
            <div className="form-group row">
              ...
              <div className="col-sm-10">
                <input
                  ...
                  value={this.props.stockNew?.description}
                  onChange={event => this.fieldChanged(event)}
                />
                <span className="invalid-feedback">
    ...
    return this.state.submitted && this.props.saved ? (
      <Redirect to={"/stockNews/" + this.props.stockNew.id} />
    ) : (
      form
    );
  }
...
const mapStateToProps = state => {
  return {
    stockNew: state.stockNew.stockNew,
    saved: state.stockNew.saved
  };
};

const mapDispatchToProps = dispatch => {
  return {
    fetchStockNew: id => dispatch(actions.fetchStockNew(id)),
    updateStockNew: stockNew => dispatch(actions.updateStockNew(stockNew)),
    resetStockNew: () => dispatch(actions.resetStockNew()),
    saveStockNew: stockNew => dispatch(actions.saveStockNew(stockNew))
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(AddStockNew);
