...
type Props = {
  resetStockRevenue: any,
  match: any,
  fetchStockRevenue: any,
  stockRevenue: any,
  saveStockRevenue: any,
  updateStockRevenue: any,
  saved: any
}

class AddStockRevenue extends Component<Props, any> {
  state = {
    title: "Add StockRevenue",
    controls: {
      name: {
        validation: {
          required: true,
  ...
  componentDidMount() {
    this.props.resetStockRevenue();
    const id = this.props.match?.params?.id;
    if (id) {
      this.setState({ title: "Edit StockRevenue" });
      this.props.fetchStockRevenue(id);
    }
  }

  fieldChanged = event => {
    const stockRevenue = {
      ...this.props.stockRevenue,
      [event.target.name]: event.target.value
    };

    this.checkFieldValidity(event.target.name, event.target.value);

    this.props.updateStockRevenue(stockRevenue);
  };
  ...
  onSubmit = event => {
    ...
    for (let fieldName in this.state.controls) {
      isValid =
        this.checkFieldValidity(fieldName, this.props.stockRevenue[fieldName]) &&
        isValid;
    }

    if (isValid) {
      this.props.saveStockRevenue(this.props.stockRevenue);
    }
  };
  ...
  render() {
            ...
            <div className="form-group row">
              ...
              <div className="col-sm-10">
                <input
                  ...
                  value={this.props.stockRevenue?.name}
            ...
            <div className="form-group row">
              ...
              <div className="col-sm-10">
                <input
                  ...
                  value={this.props.stockRevenue?.code}
                  onChange={event => this.fieldChanged(event)}
                />
                <span className="invalid-feedback">
            ...
            <div className="form-group row">
              ...
              <div className="col-sm-10">
                <input
                  ...
                  value={this.props.stockRevenue?.description}
                  onChange={event => this.fieldChanged(event)}
                />
                <span className="invalid-feedback">
    ...
    return this.state.submitted && this.props.saved ? (
      <Redirect to={"/stockRevenues/" + this.props.stockRevenue.id} />
    ) : (
      form
    );
  }
...
const mapStateToProps = state => {
  return {
    stockRevenue: state.stockRevenue.stockRevenue,
    saved: state.stockRevenue.saved
  };
};

const mapDispatchToProps = dispatch => {
  return {
    fetchStockRevenue: id => dispatch(actions.fetchStockRevenue(id)),
    updateStockRevenue: stockRevenue => dispatch(actions.updateStockRevenue(stockRevenue)),
    resetStockRevenue: () => dispatch(actions.resetStockRevenue()),
    saveStockRevenue: stockRevenue => dispatch(actions.saveStockRevenue(stockRevenue))
  };
};

export default connect(mapStateToProps, mapDispatchToProps)(AddStockRevenue);
